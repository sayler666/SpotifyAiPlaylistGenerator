...
class PlaylistGenerator:
    ...
    def generate(self, prompt: str, private: bool = False, debug: bool = False) -> Optional[str]:
        """Generate playlist and return its URL."""
        with console.status(f"[bold green]Generating playlist using {self.ai.get_name()} ({self.ai.get_model()})..."):
            # Get AI suggestions
            ...
            with Progress(
                ...
                for track in playlist_data["tracks"]:
                    ...
                    if result:
                        ...
                        console.print(f"[green]✓[/green] Found: {result.artist} - {result.name}")
                    else:
                        not_found.append(f"{track['artist']} - {track['title']}")
                        console.print(f"[red]✗[/red] Not found: {track['artist']} - {track['title']}")
